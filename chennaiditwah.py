...
import sys
import time 
if (not os.path.isfile('gfs.t06z.pgrb2.0p25.f000')):

    client = boto3.client('s3', config=Config(signature_version=UNSIGNED))
    client.download_file('noaa-gfs-bdp-pds', 'gfs.20251201/06/atmos/gfs.t06z.pgrb2.0p25.f000', 'gfs.t06z.pgrb2.0p25.f000')
u850 = xr.open_dataset('gfs.t06z.pgrb2.0p25.f000', engine='cfgrib',backend_kwargs={'filter_by_keys':{'typeOfLevel': 'isobaricInhPa', 'shortName': 'u', 'level': 850}})
u = u850.u
print(u.shape)
v850 = xr.open_dataset('gfs.t06z.pgrb2.0p25.f000', engine='cfgrib', backend_kwargs={'filter_by_keys':{'typeOfLevel': 'isobaricInhPa', 'shortName': 'v', 'level': 850}})
v = v850.v
...
mask = ((vort850.latitude <= 22) & (vort850.latitude >= 3.0) & (vort850.longitude <= 100.) & (vort850.longitude >= 70.))
...
y_ll = list(vortmask.latitude.values).index(3.0)
y_ur = list(vortmask.latitude.values).index(22.0)
...
y_ll_subset = list(vortmask.latitude.values).index(3.0)
y_ur_subset = list(vortmask.latitude.values).index(22.0)
...
plt.title("GFS 0-h 850 hPa non-divergent wind magnitude ($m s^{-1}$) due to tropical low at 06 UTC 1 December 2025")
